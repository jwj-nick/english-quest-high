# 🗡️ English Quest — High (고등학생용)

매일 5분, 던전을 클리어하며 영어 정복 — 고등학교 1~3학년 대상 RPG형 영어 자습 앱.
...
> 중학생용: [`jwj-nick/english-quest-mid`](https://github.com/jwj-nick/english-quest-mid)
...
콘텐츠 난이도: **수능 1-2등급, 한 문장 8-25 단어 · 한 지문 100-180 단어**.
...
npm run dev      # http://localhost:5173/english-quest-high/
npm run build
```

## 콘텐츠

`public/content/{YYYY-Www}/` 폴더에 주차별 JSON. 콘텐츠 생성은 상위 디렉토리(`../50_scripts/publish-week.py --target high`)에서 자동 배포.

스키마: `../01_Initial_Trial/sample/SCHEMA.md`.

## 데이터

- 학습 로그: IndexedDB `english-quest-high` (브라우저 로컬, URL 별로 분리)
