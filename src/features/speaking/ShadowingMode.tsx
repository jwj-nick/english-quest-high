...
export function ShadowingMode({ items, xpWeight }: Props) {
  ...
  if (items.length === 0) {
    return <p className="text-center text-sm text-slate-500 dark:text-slate-400 py-8">문장이 없어요.</p>
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-5 bg-gradient-to-br from-rose-50 to-white dark:from-rose-900/20 dark:to-slate-800/40 border-rose-200 dark:border-rose-700">
        <p className="text-base sm:text-lg font-semibold text-slate-900 dark:text-slate-100 leading-[1.7]">
          {current.sentence_en}
        </p>
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-3 leading-relaxed">{current.sentence_ko}</p>

        {current.phonetics_focus.length > 0 && (
          <div className="mt-4 pt-4 border-t border-rose-100 dark:border-rose-800">
      ...
      <Card className="p-4">
          ...
          {([0.65, 0.85, 1.0] as const).map((r) => (
            <button
              ...
              className={cn(
                ...
                rate === r ? 'bg-rose-500 text-white' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-600'
              )}
            >
              {r === 1.0 ? '1.0x' : `${r}x`}
            </button>
        ...
        <div className="mt-3 p-3 bg-rose-50/60 dark:bg-rose-900/20 rounded-xl border border-rose-200/60 dark:border-rose-700/60 flex items-start gap-2">
          <Mic className="h-3.5 w-3.5 text-rose-600 dark:text-rose-400 mt-0.5 flex-shrink-0" />
